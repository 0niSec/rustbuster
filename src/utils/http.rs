...
pub async fn handle_response(response: reqwest::Response) {
    ...
    if status.is_success() {
        println!("{:<DISPLAY_INDENT$} [{}]", path, status.as_str().green().bold(), DISPLAY_INDENT = DISPLAY_INDENT as usize)
    }

    if status.is_redirection() {
        println!("{:<DISPLAY_INDENT$} [{}]", path, status.as_str().yellow().bold(), DISPLAY_INDENT = DISPLAY_INDENT as usize)
    }

    if status.is_client_error() {
        println!("{:<DISPLAY_INDENT$} [{}]", path, status.as_str().red().bold(), DISPLAY_INDENT = DISPLAY_INDENT as usize)
    }
}
